...
########## setup
    ...
    for i, config in enumerate(options):
        ...
        output_path = pl.Path(f'/home/lorenz/BMDataAnalysis/final_output/graph_ml_experts_5fold_pleasebebad')
        ...
        extra_data = [c for c in data.columns if not (c.startswith('target') or c.startswith('ignored') or c.split('_')[0] in data_prefixes or 'Lesion ID' in c)]
        ...
        labels = [row['t6_rano'] for i, row in data.iterrows()]
