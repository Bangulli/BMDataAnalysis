...
def ldam_loss(input, target, dist):
    loss = LDAMLoss(dist)
...
########## setup
    ...
    for i, config in enumerate(options):
        ...
        train_data, test_data = d.load_prepro_noisy_data(data,
                                            ...
                                            target_time=360,
                                            save_processed=output.parent/'target_generation_test_data.csv')

        dist = Counter(test_data['target_rano'])
        extra_data = [c for c in train_data.columns if not (c.startswith('ignored') or c.startswith('target') or c.split('_')[0] in data_prefixes)]
        ...
        labels = [row['target_rano'] for i, row in train_data.iterrows()]
        ...
        cutoffs=[90, 120, 150, 180, 240, 300]
        all_results = {}
        for i in cutoffs:
            key_year = f"1yr_t0:{i}days->target_rano"
            # make datasets
            dataset_train = d.NoisyBrainMetsGraphClassification(train_data,
                used_timedelta = i, 
            ...
            dataset_test = d.NoisyBrainMetsGraphClassification(test_data,
                used_timedelta = i, 
                ignored_suffixes = ('_timedelta_days', '_rano', 'Lesion ID'), 
                rano_encoding = rano_encoding,
                target_name = 'target_rano',
            ...
            best_model, best_loss = torch_engine.train(model, 
                                            ...
                                            loss_function=ldam_loss if num_out>1 else F.binary_cross_entropy_with_logits,
                                            epochs=500,
                                            optimizer=optimizer,
                                            scheduler=scheduler,
