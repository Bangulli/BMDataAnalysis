...
########## setup
    ...
    for i, config in enumerate(options):
        ...
        output_path = pl.Path(f'/home/lorenz/BMDataAnalysis/final_output/graph_ml_experts_noisy_5fold_bugfix')
        ...
        data_prefixes = [f"t{k}" for k in range(38)] # used in the training method to select the features for each step of the sweep
        ...
        extra_data = [c for c in data.columns if not (c.startswith('target') or c.startswith('ignored') or c.split('_')[0] in data_prefixes or c=='Lesion ID')]
        ...
        all_results = {}
        all_stds = {}
        dts = [0, 90, 150, 210, 270, 330]
        for i in dts:
            key_year = f"1yr_t0:{i}->target_rano"
            skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)

            all_folds = []
            for fold, (train_idx, val_idx) in enumerate(skf.split(data, data['target_rano'])):
                print(f"Fold {fold+1}")
                wdir = output/key_year/f"Fold {fold+1}"
                ...
                # evaluate
                best_res = torch_engine.test_classification(best_model, dataset_test, wdir, device, rano_encoding, num_out==1)
                ...
                all_folds.append(best_res)

                # plot
                plot_prediction_metrics(best_res, wdir)
            print('prasing folds')
            interim = {}
            ...
            all_results[key_year] = interim
            all_stds[key_year] = interim_std
            #print(all_results, all_stds)
        plot_prediction_metrics_sweep(all_results, output, classes=classes, distribution=dist, std=all_stds)
