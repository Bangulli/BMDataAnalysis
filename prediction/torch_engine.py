...
def train(model, dataset, epochs=200, loss_function=F.cross_entropy, optimizer=adam, scheduler=None, working_dir=None, device='cuda', validation=0.25, batch_size='all', verbose=False, weighted_loss=True):
    ...
    with open(working_dir/'assignments.csv', 'w') as file:
        ...
        for batch in loader:
            ...
            if not binary: 
                ...
                assignment = (confidence >= optimal_threshold).long().cpu().numpy()
                confidence = confidence.cpu().numpy()

            for pd, gt, conf in zip(assignment, y, confidence):
                res = {'prediction': pd, 'target': gt, 'confidence':conf}
                writer.writerow(res)
    return classification_evaluation(assignment, y, rano_encoding)
