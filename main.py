...
if __name__ == '__main__':
    dataset_path = pl.Path('/mnt/nas6/data/Target/batch_copy/reseg_test/set')#'/mnt/nas6/data/Target/PROCESSED_mrct1000_nobatch')
    met_path = pl.Path('/mnt/nas6/data/Target/batch_copy/reseg_test/parsed_mets_new')#'/mnt/nas6/data/Target/PARSED_METS_mrct1000_nobatch')
    print("== I am currently set to run on the test set")
    ...
    for pat in pats:
        ...
        for i, series in enumerate(p.mets):
            new = series.resample(method='nearest')
            if new is not None:
                print('nearest neighbor resampled series:', i)
                new.print()
                #new._plot_trajectory_comparison(f"comparison - patient-{pat}_metastasis-{i}_nn_interpolation.png", series)

            new = series.resample(method='linear')
            if new is not None:
                print('linear resampled series:', i)
                new.print()
                #new._plot_trajectory_comparison(f"comparison - patient-{pat}_metastasis-{i}_linear_interpolation.png", series)

            new = series.resample(method='bspline')
            if new is not None:
                print('bspline resampled series:', i)
                new.print()
                #new._plot_trajectory_comparison(f"comparison - patient-{pat}_metastasis-{i}_bspline_interpolation.png", series)
