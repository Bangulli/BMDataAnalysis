...
for file in [f for f in os.listdir(source) if (source/f).is_dir()]:
    for fold in folds:
        if file not in list(combined.keys()):
            combined[file] = pd.read_csv(source/file/fold/'assignments.csv')
        else:
            combined[file] = pd.concat([combined[file], pd.read_csv(source/file/fold/'assignments.csv')], ignore_index=True)
