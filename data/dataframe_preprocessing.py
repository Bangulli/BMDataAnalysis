...
def load_prepro_data(path, 
    ...
    rano_cols = [f"{prfx}_rano" for prfx in prefixes if prfx != 't0']
    volume_cols = [f"{prfx}_volume" for prfx in prefixes]
    drop_rows = []
    ...
    df = apply_interpolation_for_target_CR(df, interpolate_rows, interpolate_CR_swing_length)
...
def apply_interpolation_for_target_CR(df, targets, threshold):
    vol_cols = ["t0_volume", "t1_volume", "t2_volume", "t3_volume", "t4_volume", "t5_volume", "t6_volume"]
...
if __name__ == '__main__':
    d, _ = load_prepro_data('/mnt/nas6/data/Target/BMPipeline_full_rerun/229_patients faulty/PARSED_METS_task_502/csv_nn/features.csv',
                         drop_suffix=None, used_features=['volume'], normalize_volume=None, normalize_suffix=None)
    print(d)
    d.info()
    print(d.describe())
